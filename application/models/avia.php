...
/*	---------------------------------------------------------------------
 *	Original class Mdl version 1.0
 *	Codeigniter 2.1.2
 *	Created By @ahmadhendri21
 *	Rilis Date : 23 Juli 2015 at Payakumbuh, Sumatera Barat Indonesia
 *  republish GoSitus - Avia Tour
 *	--------------------------------------------------------------------- */ 

class Avia extends CI_Model
{
	/* 
		array(
			select => 'string',
			filter => array(),
			like	=> array()
			order	=> array() OR string id|DESC
			limit	=> array() OR string 5|0
			join	=> array(tabel=>condition),
			multi_join => array(
				array('tabel'=>condition)
			)
		)
	*/
	...
	public function a($a,$b='*'){return $this->db->select($b)->get($a);}
	public function ad($a,$o,$b='*'){return $this->db->select($b)->order_by($o,'DESC')->get($a);}	
	public function f($a,$c,$b='*'){return $this->db->select($b)->get_where($a,$c);}
	public function fd($a,$c,$o,$b='*'){return $this->db->select($b)->order_by($o,'DESC')->get_where($a,$c);}
	public function s($a,$c,$b='*'){return $this->db->select($b)->like($c)->get($a);}
	public function sd($a,$c,$o,$b='*'){return $this->db->select($b)->like($c)->order_by($o,'DESC')->get($a);}	
	public function la($a,$d=5,$e=0,$b='*'){return $this->db->select($b)->get($a,$d,$e);}
	public function lad($a,$o,$d=5,$e=0,$b='*'){return $this->db->select($b)->order_by($o,'DESC')->get($a,$d,$e);}	
	public function lf($a,$c,$d=5,$e=0,$b='*'){return $this->db->select($b)->where($c)->get($a,$d,$e);}
	public function lfd($a,$c,$o,$d=5,$e=0,$b='*'){return $this->db->select($b)->where($c)->order_by($o,'DESC')->get($a,$d,$e);}	
	public function ls($a,$c,$d=5,$e=0,$b='*'){return $this->db->select($b)->like($c)->get($a,$d,$e);}
	public function lsd($a,$c,$o,$d=5,$e=0,$b='*'){return $this->db->select($b)->like($c)->order_by($o,'DESC')->get($a,$d,$e);}	
	public function ca($a){return $this->db->count_all($a);}		
	public function cf($a,$c){return $this->db->where($c)->count_all_results($a);}
	public function cs($a,$c){return $this->db->like($c)->count_all_results($a);}
	public function i($a,$i){$this->db->insert($a,$i);return $this->db->insert_id();}		
	public function u($a,$u,$c){return $this->db->update($a,$u,$c);}		
	public function d($a,$c){return $this->db->delete($a,$c);}
...
/* End of file mdl.php */
/* Location: ./application/model/mdl.php */